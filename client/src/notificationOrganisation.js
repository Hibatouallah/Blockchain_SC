...
class notificationOrganisation extends Component {
    ...
    componentDidMount = async () => {
        ...
        var nb =  await organisation.methods.getnbnotification_organisation().call()
        this.setState({nbnotification:nb})
       
        for (var i=0; i < nb; i++) {
            const stat = await organisation.methods.getstatus_organisation(i).call()
            if(stat == "Disponible"){
                const msg = await organisation.methods.getmessage_organisation(i).call()
                const list =[{
                    message: msg,
                    numnotification : i,
                    status : stat
...
export default notificationOrganisation
